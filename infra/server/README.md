...
- Build lỗi/OOM: container ghi `buildFailed: true`, ngủ 10 phút rồi Docker khởi động lại; xem log và status, chờ marker lỗi rồi chạy `pnpm data:rollback`, hoặc rollback graph thủ công với đủ `--expected-current`/`--expected-target` lấy từ status. CLI kiểm lại hai identity dưới cùng kernel lock ngay trước transaction, nên status cũ không thể làm graph bị toggle về phía trước.
