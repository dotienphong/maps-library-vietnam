...
5. **Cảnh báo tunnel down (bắt buộc, spec 2026-09-20-canh-bao-suc-khoe mục 4)**: chính sách Notification `tunnel_health_event` lọc trạng thái **Down**, email tới địa chỉ vận hành. Tạo trên Dashboard → Notifications → Add → Cloudflare Tunnel → Tunnel Health Alert (token API của máy dev không có `Notifications Write`, đã thử 20/09/2026). Email mới phải bấm xác nhận trong thư của Cloudflare. Chi tiết và trạng thái: `docs/evidence/health/2026-09-20-canh-bao-suc-khoe.md`.
