...
- **Production đang đặt `VALHALLA_THREADS=2` (22/09/2026).** Lý do: với 1 luồng, năm request `/v1/matrix` cỡ tối đa chạy song song đẩy p95 của `/v1/directions` từ ~0,8 s lên 2,7–5,3 s — ma trận xếp hàng trước tuyến của khách khác (số đo `docs/evidence/routing/2026-09-22-matrix.md`). Đổi biến rồi `docker compose -f infra/server/compose.yml --env-file infra/server/.env up -d valhalla`: container nạp lại tar có sẵn, **không** build lại graph, `/v1/directions` trả 503 khoảng một phút. Sau khi lên, kiểm `/custom_files/` không còn `reload.failed` hay `reload.in-progress`.
