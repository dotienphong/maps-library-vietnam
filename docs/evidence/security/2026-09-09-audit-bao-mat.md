...
**Bước E, 09/09 ~22:00, không cần dashboard:** khoá S3 suy từ `CLOUDFLARE_API_TOKEN` của máy chủ (id token + sha256) đọc/ghi được cả hai bucket (đã thử trong container). File mã hoá 21:09 đã `rclone moveto` sang `mapslibvn-backups/backups/daily/`; bucket tiles không còn `backups/`. PHONG đã chạy ~22:00: `node scripts/server-r2-creds.mjs` → recreate `backup`,`pipeline` → `backup.mjs --once` → bucket riêng có `…-2109` và `…-2201.dump.zst.enc` (947 MB, giải mã + `zstd -t` OK). **E xong.** Ghi chú: hai lần `--once` trong cùng một phút ghi trùng tên file tạm nên lần sau báo "no such file" — đã sửa bằng tên file tạm kèm pid. Việc tuỳ chọn còn lại: xoá token S3 cũ (chỉ bucket tiles) trên dashboard R2. Ghi chú: bản upload plaintext 20:30 vào bucket riêng bằng wrangler đã KHÔNG thành công (file quá lớn), và bản tạm trên máy dev đã xoá — bản backup còn lại là file mã hoá. Token S3 cũ (chỉ bucket tiles) nên xoá trên dashboard khi tiện.
