...
- [~] **9.3** Đã push đúng remote và theo dõi workflow thật theo SHA. Đường đi 07/09: `CI` đỏ ở `178d086`/`0585eb1` vì 5 test `styleUrl` của việc `sources` (phiên đó sửa ở `37e4870`); **`834db37` xanh 4/5** (CI, Deploy API, Deploy Docs, API tests DB thật — `DB tests` còn chạy). `bde2d35` lại đỏ `CI` vì một lỗi **thứ tự build**: `scripts/lib/poi-profile.mjs` → `pipelines/.../poi-filter.mjs` → `@mapslibvn/core`, mà `tsc -p tsconfig.scripts.json` chạy **trước** `turbo run typecheck` nên không được hưởng `dependsOn: ["^build"]`; local xanh chỉ vì `packages/core/dist` đã build sẵn, tái hiện được bằng `rm -rf packages/core/dist && pnpm typecheck`. Sửa ở `44da644`. **Chờ workflow trên `44da644` chạy xong để ghi run URL + conclusion thật.** Ghi nhận thêm một **lỗ hổng quy trình cần PHONG quyết**: `deploy-api.yml` chỉ `on: push` và **không `needs:`** job API tests, nên trên cả `178d086` và `0585eb1`, Deploy API thành công dù test DB thật đỏ ~10 giây trước — đó là đường mà sự cố 503 lên được production.
