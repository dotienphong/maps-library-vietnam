...
- [~] **8.3** ĐÃ CHẠY CỔNG TRÊN PRODUCTION (06:40Z 07/09 — lần đầu; trước đó cổng chỉ chạy trên DB đo `mapslibvn_alias_scale` **trước** publish) rồi chạy lại trên DB staging dựng từ production để đo bản sửa — [hồ sơ](../../evidence/admin-alias/8-3-8-4-coverage-production.md), [số liệu](../../evidence/admin-alias/8-3-8-4-coverage-production.json). **Đã sửa một lỗi thật: 8 vùng cũ đất liền mất sạch alias.** `admin-overlay.mjs` bỏ mọi khoá có nhiều hơn một chủ, mà bốn cặp chỉ khác nhau ở dấu (Đông Thạnh/Đông Thành, Sa Pa/Sa Pả, Phú Thành/Phú Thạnh, Lộc Thạnh/Lộc Thành) trùng `name_norm` **kể cả ở khoá đầy đủ nhất** nên cả hai phía mất hết đường tra — trái quyết định #3 ("không gộp mất provenance `old_area_id`"). Bản sửa giữ khoá đầy đủ nhất, vẫn bỏ khoá ngắn nhập nhằng, dedupe theo PK ba cột; thêm report `ambiguousPrimaryKept` để công bố đánh đổi. Cổng **84 → 79 failure**, `missing_mainland_l8` **10 → 5**, alias 37.246 → 37.251. Còn lại **không phải việc code**: `count_out_of_range` (L8 4.215 so với 10.000–10.700 — lệch nguồn cực không đều: Sơn La **1** xã cũ, Bắc Giang 2, Vĩnh Phúc 2), `raw_coverage_gap` 60 (ven biển, mẫu số gồm lãnh hải), 1.970 cảnh báo `discarded_sliver`, và 3 ca `missing_mainland_l8` chỉ hết nếu `old_area_id` vào PK (migration 0009 — plan cấm ở phạm vi này). **CHƯA publish lên production** (thuộc 9.2): production vẫn thiếu alias 8 vùng đó. **Đã chuẩn bị số cho các quyết định** — [hồ sơ](../../evidence/admin-alias/8-3-decision-prep.md): 60 `raw_coverage_gap` **đều là nước** (42 vùng ≤ 0,5 POI/km², 13 vịnh có cầu tàu, 5 ca Cát Bà/Tam Giang ≤ 2,3 POI/km²), không có lỗ đất liền; 92 % sliver là cạnh < 0,001, chỉ 11 vùng bỏ ≥ 5 %; hạ ngưỡng xuống 0,005 đổi tập đích 337 vùng mà `hn-01` **vẫn đỏ** vì Văn Miếu 0,130 ở lại. **PHONG đã duyệt A+B+C 07/09/2026 và đã cài** (`149bb1f`): overlay đo mật độ POI phần trống/phần phủ cho vùng thiếu phủ, evaluator chuyển sang cảnh báo `coastal_gap_accepted` khi ≤ 0,5 POI/km² hoặc < 10% mật độ phần phủ; cảnh báo sliver từ 0,01; giữ ngưỡng 0,05. **Đã publish lên production** bằng `admin-old.mjs --accept-qa` (alias 37.246 → 37.251, không chạm `admin_area`) và chạy lại cổng: **84 → 24 failure**, cảnh báo 1.970 → 286 (`discarded_sliver` 229 + `coastal_gap_accepted` 55). `raw_coverage_gap` 60 → **5** (Cát Hải, Gia Luận, Việt Hải, Nghĩa Lộ, Hương Phong — Gia Luận/Việt Hải có mật độ phần trống **cao hơn** phần phủ nên có thể là rừng Cát Bà chưa thuộc xã nào, tức lỗ thật; giữ đỏ là đúng). 24 failure còn lại: **21 cùng gốc nguồn dữ liệu cấp xã cũ, 5 cần nguồn ranh giới Cát Bà/Tam Giang, 0 lỗi code** — mục F [hồ sơ](../../evidence/admin-alias/8-3-decision-prep.md).
- [~] **8.4** ĐÃ CHẠY LẠI trên staging sau sửa: **48/60 ca đạt, ca tách 4/6** (mốc ≥5 chưa đạt). **Phân rã lại 12 ca trượt cho thấy kết luận cũ của plan là sai: 10/12 KHÔNG phải fixture ghi sai.** `lc-01`…`lc-10`: snapshot **không có xã cũ nào** của Huyện Than Uyên (raw chỉ có duy nhất relation `Than Uyên` cấp 6; Lai Châu có L8=43 nhưng không xã nào của huyện này), trong khi mọi xã đích mới đều có trong `admin_area` — thiếu dữ liệu nguồn, cùng gốc với `count_out_of_range`. `hn-01`: overlay đúng hình học nhưng **ngưỡng sliver 0,05 loại đúng hai đích nghị quyết có nêu** (Hoàn Kiếm 0,029, Ba Đình 0,008), còn Văn Miếu–Quốc Tử Giám 0,130 thì nghị quyết không nêu. `ct-01`: Bình Thủy 0,623 + Cái Khế 0,302 đúng nghị quyết, thêm Thới An Đông 0,075 vượt ngưỡng nên được giữ — lệch niên đại ranh giới snapshot 01/2025 so với hiện hành. `fixture_district_mismatch` đã về 0 từ `d1bc08c`, tức lớp lỗi "fixture sai huyện" đã đóng. **Không ca nào trong 12 ca được giải bằng cách đọc lại nghị quyết.** Cần PHONG quyết: nguồn dữ liệu cấp xã, và ngưỡng `share ≥ 0,05` (plan đã chốt tham số này).
