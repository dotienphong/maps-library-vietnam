...
- [~] **9.4** ĐÃ CHẠY trên production 07/09 — [hồ sơ](../../evidence/admin-alias/9-4-9-5-nghiem-thu-production.md). ĐẠT: coverage **0 failure/309 warning/exit 0**; 60 ca fixture 48/60 + 12 ca khai báo có bằng chứng; 10 cặp geocode 9/10; benchmark theo 8.5; **API role đọc old table** (`api` có SELECT trên admin_area/old/alias/poi/street/category, `/healthz/db` trả `user: api`); **export ODbL có old+alias mới** (3.353 / 4.972 / **37.251**, manifest có sha256 từng bảng); **playground tải SDK mới** (`mapslibvn.umd.js` có `"area"`, 7× `poiSources`); smoke `Quận 10` / `Bình Dương` / `Thủ Dầu Một` đúng, `types` loại trừ area cho `area=0`. **MỞ vì một ca smoke lộ vấn đề thật**: truy vấn **tên hiện hành kèm tiền tố đơn vị** làm khớp chính xác tụt hạng — `Phường Diên Hồng` xuống hạng 2 (top là Phường An Khánh), `Phường Bàn Cờ` **không có trong 10**; bỏ tiền tố thì đúng hạng 1. Nghi token dùng chung `phuong` làm `word_similarity` cao giả; không đều (`Phường Sài Gòn`, `Xã Chợ Vàm` vẫn đúng). Không phải lỗi dữ liệu alias, không phải hồi quy hôm nay — cần điều tra riêng về xếp hạng nhánh area. **Trong lúc chạy 9.4 phát hiện sự cố production 4 endpoint 503** (`malformed array literal` do bind mảng nguồn POI); đã sửa ở `2728347`, `test:api-db` 7 đỏ → 33 xanh, deploy xong và cả 4 endpoint về 200 — chi tiết mục 0 hồ sơ.
