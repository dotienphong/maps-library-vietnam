...
**Năm việc đã phát hiện — bốn đã sửa hoặc sửa một phần, một còn chặn:**
...
3. **[SỬA MỘT PHẦN — còn cần dựng lại current]** **Thiếu Khánh Hòa.** OSM không có relation `admin_level=4` cho Khánh Hòa ở cả snapshot 01/2025 lẫn OSM hiện tại. Nhưng **các đơn vị cấu thành thì có đủ**: snapshot có 8 quận/huyện cũ (Nha Trang, Cam Ranh, Cam Lâm, Diên Khánh, Khánh Sơn, Khánh Vĩnh, Vạn Ninh, Ninh Hòa), và Overpass xác nhận OSM hiện tại có các phường mới (Nha Trang, Bắc/Nam/Tây Nha Trang, `admin_level=6`). Đã thêm `bootstrapMissingProvince()` dựng L4 bằng **hợp các con mồ côi**, với hai chốt an toàn: chỉ chạy khi **đúng một** tỉnh trong `provinces.json` thiếu (đo thật: đúng một, `khanh hoa`, ở cả hai phía), và chỉ gộp con có **≥90% diện tích trong VN** — ngưỡng theo số đo `Sa Mouay` (Lào) 0,362 so với Khánh Hòa 0,987–1,000; kiểm bằng point-on-surface là **không đủ** vì `vn_boundary` đệm 2 km nên Sa Mouay vẫn lọt. Chạy thật trên snapshot toàn quốc: L4 **62 → 63** (đúng spec), L6 **686 → 694** (vào khoảng 690–710), L8 4.152 → 4.215, Khánh Hòa có 72 vùng cũ. **Cảnh báo vận hành:** sửa riêng nhánh old làm cổng QA **xấu đi** (unmatched 72 → 142) vì phía current vẫn thiếu tỉnh đó. Phải dựng lại current bằng cùng hàm này mới xanh được — cần extract OSM hiện hành (`work/vietnam-patched.osm.pbf`, ~400 MB) và một lần chạy `admin.mjs` toàn quốc. **Không publish trạng thái trung gian.**
