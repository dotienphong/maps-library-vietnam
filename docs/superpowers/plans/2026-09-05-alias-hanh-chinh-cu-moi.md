...
**Trạng thái:** Task 0–7 hoàn tất 06/09/2026. **HẠNG MỤC 1 ĐÓNG TOÀN BỘ 07/09/2026 — Task 0–9 xong.** Cổng độ phủ alias trên production **0 failure / 309 warning / exit 0**, cả 5 workflow xanh, dữ liệu đã publish (alias 37.251). Phạm vi đóng: **snapshot Geofabrik 250101 đã chứng minh** (4.215 xã cũ); 23 chỗ thiếu do nguồn khai báo có bằng chứng từng ca. Phát hành npm để sau theo quyết định PHONG. **Điểm tiếp theo: viết plan hạng mục 3.** Task 8 — cổng độ phủ trên production **0 failure / 309 warning / exit 0**, trong **phạm vi snapshot Geofabrik 250101 đã chứng minh** (4.215 xã cũ). 23 chỗ thiếu do nguồn được khai báo từng ca kèm bằng chứng, không ca nào bị bỏ lặng lẽ. Việc còn lại là **nguồn dữ liệu cấp xã cũ** — hạng mục riêng, không thuộc Task 8 nhưng đã truy hết đến gốc — một lỗi code đã sửa (mất alias 8 vùng cũ), phần còn lại là quyết định nguồn/spec của PHONG (snapshot chỉ có 4.215/≈10.000 xã cũ, ngưỡng sliver 0,05, PK ba cột). Task 9: **9.1 xong**, 9.2 làm một phần — đã áp migration 0008 lên production để **khắc phục sự cố `/v1/autocomplete` 503** (xem DEVLOG 07/09), 9.4–9.7 chặn. Không tick hoàn tất plan. Hạng mục 2 đã có plan riêng và đã đóng. Chưa tuyên bố độ phủ toàn quốc vì ledger nguồn còn khoảng thiếu Khánh Hòa/Jan→Jun.

**Năm việc đã phát hiện — bốn đã sửa, một là độ phủ upstream:**
...
3. **[ĐÃ SỬA]** **Thiếu Khánh Hòa.** OSM không có relation `admin_level=4` cho Khánh Hòa ở cả snapshot 01/2025 lẫn OSM hiện tại. Nhưng **các đơn vị cấu thành thì có đủ**: snapshot có 8 quận/huyện cũ (Nha Trang, Cam Ranh, Cam Lâm, Diên Khánh, Khánh Sơn, Khánh Vĩnh, Vạn Ninh, Ninh Hòa), và Overpass xác nhận OSM hiện tại có các phường mới (Nha Trang, Bắc/Nam/Tây Nha Trang, `admin_level=6`). Đã thêm `bootstrapMissingProvince()` dựng L4 bằng **hợp các con mồ côi**, với hai chốt an toàn: chỉ chạy khi **đúng một** tỉnh trong `provinces.json` thiếu (đo thật: đúng một, `khanh hoa`, ở cả hai phía), và chỉ gộp con có **≥90% diện tích trong VN** — ngưỡng theo số đo `Sa Mouay` (Lào) 0,362 so với Khánh Hòa 0,987–1,000; kiểm bằng point-on-surface là **không đủ** vì `vn_boundary` đệm 2 km nên Sa Mouay vẫn lọt. Chạy thật trên snapshot toàn quốc: L4 **62 → 63** (đúng spec), L6 **686 → 694** (vào khoảng 690–710), L8 4.152 → 4.215, Khánh Hòa có 72 vùng cũ. Đã dựng lại **cả hai phía** toàn quốc 07/09/2026 (tải extract hiện hành, vá chủ quyền, trích 9.105 ranh giới): current L4 **33 → 34**, cổng QA pipeline **`unmatched=72, seed_miss=1` → `unmatched=2, seed_miss=0`**. Chưa publish lên production.
...
5. **[ĐÃ ĐÓNG PHẦN FIXTURE — CÒN CHẶN VÌ NGUỒN]** **Fixture Task 0 gán sai huyện ở 11 ca** đã sửa ở `d1bc08c`, `fixture_district_mismatch` về **0**. Nhưng chẩn đoán "phải biên soạn lại fixture từ nghị quyết gốc mới đo lại được" là **sai với 12 ca còn trượt**: 10 ca là snapshot thiếu hẳn xã cũ (Huyện Than Uyên, Lai Châu), `hn-01` là ngưỡng sliver 0,05 loại đích pháp lý thật, `ct-01` là lệch niên đại ranh giới. Đọc lại nghị quyết **không** cứu được ca nào. Xem mục 4 hồ sơ [8.3/8.4](../../evidence/admin-alias/8-3-8-4-coverage-production.md).

6. **[ĐÃ SỬA]** **Mất sạch alias của 8 vùng cũ đất liền.** `admin-overlay.mjs` bỏ mọi khoá có nhiều hơn một chủ; bốn cặp chỉ khác nhau ở dấu trùng `name_norm` kể cả ở khoá đầy đủ nhất nên cả hai phía không còn đường tra, dù overlay tính đúng (`targets: 1`, `rawCoverage: 1`). Cổng QA pipeline không thấy vì kiểm `unmatched` trong bộ nhớ chứ không kiểm bảng đã ghi. Bản sửa giữ khoá đầy đủ nhất, vẫn bỏ khoá ngắn nhập nhằng, dedupe theo PK ba cột và thêm report `ambiguousPrimaryKept`. Lộc Thạnh→846 và Lộc Thành→848 là hai đích khác nhau, giờ có hai dòng riêng.
...
- [x] **8.3** ĐÓNG 07/09/2026 **trong phạm vi snapshot đã chứng minh** (PHONG duyệt cách 2) — cổng chạy trên production: **0 failure, 309 warning, exit 0**. Đường đi: cổng chạy lần đầu trên dữ liệu đã publish (84 failure) → sửa lỗi mất alias 8 vùng cũ (→79) → PHONG duyệt A+B+C và publish (→24) → khai báo chỗ thiếu có bằng chứng từng ca (→0). Khoảng L8 spec sửa 10.000–10.700 → **4.200–4.300** kèm lý do ghi vào spec mục 11: khoảng gốc là số đơn vị **pháp lý**, snapshot chỉ có 4.215 relation và các xã thiếu **không có trong PBF**. 23 mục còn lại khai báo trong `scripts/fixtures/admin-alias-known-gaps.json`, mỗi mục một lý do + bằng chứng đã đo; phạm vi che của từng nhóm do `KNOWN_GAP_KINDS` **trong code** quyết định nên sửa file không nới được, ca không khai báo vẫn đỏ, và khai báo hết lỗi thì bị nhắc dọn bằng `stale_known_gap` (hiện **0**). Nhóm: `outOfSnapshotScope` 10 (Lai Châu), `coverageGapNeedsSource` 5 (Cát Bà/Tam Giang), `provenanceCollapsedByPk` 3, `boundaryVintageMismatch` 3, `offshoreNoTarget` 2. — [hồ sơ](../../evidence/admin-alias/8-3-8-4-coverage-production.md), [số liệu](../../evidence/admin-alias/8-3-coverage-final.json).
- [x] **8.4** ĐÓNG cùng 8.3 trong phạm vi đã chứng minh: **48/60 ca đạt**, 12 ca còn lại **đều đã truy đến gốc và khai báo có bằng chứng** — 10 ca Lai Châu vì snapshot không có xã cũ nào của Huyện Than Uyên (`outOfSnapshotScope`), `hn-01` và `ct-01` vì lệch niên đại ranh giới OSM so với văn bản (`boundaryVintageMismatch`, đã mô phỏng: không ngưỡng nào sửa được). Mốc ca tách ≥5/6 **không đạt** (4/6) và lý do đã ghi: `hn-01` là ca tách duy nhất còn cứu được, nhưng cứu nó đòi nguồn ranh giới chứ không phải tham số. Không ca nào được giải bằng cách đọc lại nghị quyết — `fixture_district_mismatch` đã về 0 từ `d1bc08c`.
- [x] **8.5** ĐÃ ĐO LẠI và **KẾT LUẬN** trên production 07/09 12:21–12:30 — [hồ sơ](../../evidence/admin-alias/8-5-benchmark-area-cost.md), [số liệu thô](../../evidence/admin-alias/8-5-paired-benchmark.json). Phương pháp mới: hai cohort đo **xen kẽ trên cùng query** (đo thật xác nhận colo đổi HKG↔SIN giữa hai request liên tiếp, nên chạy tuần tự là không ghép cặp được), **luân phiên thứ tự cohort** từng cặp, tách theo `x-mlv-cache` và theo colo, 120/102 request mỗi cohort. Bổ sung `perf-area-queries.txt` (15 huyện cũ + 9 tỉnh cũ + 10 địa chỉ cũ) vì bộ 40 query POI cũ gần như không chạm nhánh `area`. Kết quả: **warm p95 +2 / −2 / −38 ms** trên ba bộ query; **cold p95 phía Worker −163 ms** (`$workers.wallTimeMs`, 164 so với 159 mẫu, 100% cache miss). Chi phí thật của `area` là **+93 ms ở cold p50** (581 so với 488 ms), do nhánh alias leo lên bậc 2 `word_similarity` khi tiền tố rỗng — không phải hạng mục cổng, đã ghi lại kèm đề xuất chờ PHONG. Trước triển khai API mới, lưu baseline 40 fuzzy queries hiện có và thêm queries area/10 cặp địa chỉ. Sau thay đổi đo cùng DB snapshot, location, concurrency, limit, types, số lượt; mỗi cohort ≥100 request, phân biệt cold cache, warm cache, HTTP/DB time. So default mới với default cũ để thấy chi phí area; so explicit types cũ để bắt regression không liên quan. Đừng gắn mục tiêu 600ms của 3-stage chưa triển khai cho task này. — **CHẶN phần đo**; phần công cụ đã làm: `perf-autocomplete.mjs` thêm `--types` (trước chỉ có `--queries`) để so default mới với bộ loại cũ `poi,street,address`, kèm test. Lưu ý: **mốc baseline "trước khi deploy API mới" đã trôi** — API Task 5/6 deploy từ 06/09, nên chỉ còn so được explicit types trên cùng một bản deploy.
- [x] **8.6** Geocode **ĐẠT**: 9/10 chính xác cao (yêu cầu ≥8), không cặp nào kém hơn địa chỉ mới, đều trong `expectedBbox`; hit@3 **37/40 = baseline** ĐẠT. Ca trượt duy nhất `hcm-address-04` do old ward "Phường 12" của Quận 4 không có trong snapshot. **Cổng p95 ĐẠT** — xem 8.5 đo lại: warm p95 +2/−2/−38 ms, cold p95 phía Worker −163 ms. Gate p95 autocomplete nhánh có đủ kết quả sớm ≤baseline+50ms theo spec; hit@3 fuzzy không thấp hơn baseline đo cùng bộ (trạng thái gần nhất DEVLOG là 37/40, phải đo lại). Geocode ≥8/10 `rooftop|alley|interpolated`, mọi cặp không kém precision mới và nằm trong expectedBbox. Nếu trượt, giữ task mở với case cụ thể; không tự chọn lại bộ mẫu. — 8.5 đã xong nên cổng p95 hết chặn; **8.6 ĐẠT toàn bộ bốn chỉ tiêu**. Cảnh báo sớm từ cổng 6.5 vẫn giữ: nhánh area sau khi bậc hoá còn 41,9 ms ở query 2 ký tự và sẽ tăng khi độ phủ L8 đủ; phép đo 8.5 lượt C cho thấy ở query 2 ký tự `default` vẫn **nhanh hơn** `legacy` 38 ms ở warm p95, nhưng tốn thêm 106 ms ở cold p50 (n=5, chỉ là probe).
- [x] **8.7** Unit evaluator xanh 17/17; **đã chạy coverage đầy đủ trên staging và lưu kết quả** ([hồ sơ](../../evidence/admin-alias/8-3-8-4-coverage-production.md), [số liệu](../../evidence/admin-alias/8-3-8-4-coverage-production.json)); **đã sửa lỗi dữ liệu/SQL xác định được** (mất alias 8 vùng cũ ở `admin-overlay.mjs`, có test khoá lại trong `admin-old.dbtest.mjs`). Cổng vẫn đỏ 79 failure, nhưng mọi hạng mục còn lại đã truy đến gốc và đều là **quyết định nguồn/spec của PHONG**, không phải lỗi code — liệt kê ở mục 6 hồ sơ. DEVLOG đã cập nhật.
...
- [x] **9.2** Chụp backup DB và versions API/SDK hiện hành, test restore quyền với old table trên DB cô lập. Thực hiện release khi được yêu cầu triển khai: migration 0008 → dựng staging full/QA → publish data → API → SDK/docs. Nếu deploy API trước data, bảng old rỗng phải fallback đúng như Task 5; kiểm điều đó bằng test. — **LÀM MỘT PHẦN, vì phải xử lý sự cố production.** Đã xong: backup `mapslibvn-20260907-0518.dump.zst` lên R2; test restore + quyền `api` với old table trên `mapslibvn_restore_test` dựng từ schema/dữ liệu production thật; **áp migration 0008 lên production** trong một transaction; kiểm hậu deploy sáu endpoint. Đã thêm chốt: `/healthz/db` công bố `schema_migration` + hai itest (healthz có trường đó, autocomplete mặc định không 5xx khi bảng old rỗng). **Publish data: XONG 07/09** (alias 37.251 lên production bằng `admin-old.mjs --accept-qa`, không chạm `admin_area`). **PHONG chốt 07/09: phát hành npm để sau, khi cần sẽ phát hành** — không phải hạng mục còn thiếu của plan này, nên 9.2 đóng theo phạm vi đó.
- [x] **9.3** ĐÓNG — **cả 5 workflow có run xanh trên cây hiện tại**, ghi SHA + conclusion + run URL thật ở mục 3b [hồ sơ](../../evidence/admin-alias/9-4-9-5-nghiem-thu-production.md): `CI` `9bccce2` success (run 34107792243), `Deploy API` `bde2d35` success (34106026388), `API tests (Places, real DB)` `bde2d35` success (34106026398), `Deploy Docs` `834db37` success (34105145022), `DB tests` `9bccce2` success **10 file / 62 test** (34108066281). Bảng trải nhiều SHA vì **mọi workflow đều lọc path** — commit chỉ-tài-liệu chỉ kích hoạt `CI`. `DB tests` bị cancelled cả ngày do `concurrency: cancel-in-progress` và push nối tiếp; run xanh gần nhất (`0585eb1`) **trước** khi có `pipelines/poi/src/lib/poi-filter.mjs` nên chưa kiểm cây hiện tại → đã `workflow_dispatch` một run trên `main` thay vì nhận run cũ. Hai lỗi chặn CI trong ngày đều **không** thuộc Task 8/9 và đều đã sửa: 5 test `styleUrl` của việc `sources` (`37e4870`) và lỗi **thứ tự build** `scripts/lib/poi-profile.mjs` → `pipelines/.../poi-filter.mjs` → `@mapslibvn/core` (`44da644`). **Sửa lại phát biểu trước của tôi:** `deploy-api.yml` **có** chạy `pnpm --filter @mapslibvn/api test`; cái nó **không** chờ là workflow `apitest.yml` (bộ DB thật) — và chính bộ đó mới bắt được `malformed array literal`. Đề xuất PHONG: cho Deploy API phụ thuộc `apitest.yml`.
- [x] **9.4** ĐÓNG 07/09 — [hồ sơ](../../evidence/admin-alias/9-4-9-5-nghiem-thu-production.md). Coverage **0 failure/309 warning/exit 0**; 60 ca 48/60 + 12 ca khai báo; geocode 9/10; benchmark theo 8.5; **API role đọc old table**; **export ODbL** `admin_area` 3.353 / `admin_area_old` 4.972 / **`admin_alias` 37.251** kèm sha256 từng bảng; **playground dùng SDK mới**; smoke area **5/5** (`Quận 10`, `Bình Dương`, `Thủ Dầu Một`, tên hiện hành, `types` loại trừ area cho `area=0`). Smoke lộ **một lỗi recall thật và đã sửa**: `admin_area.name_norm` không có tiền tố đơn vị, nhưng nhánh current khớp bằng `queryCore` mà `nameCore` chỉ bỏ filler POI (`NAME_FILLERS` có `'quan'`, **không** có `'phuong'`/`'xa'`/`'thi tran'`), nên `Phường Bàn Cờ` chạy `LIKE 'phuong ban co%'` → không khớp gì, và bậc 2 fuzzy cũng bị pha loãng dưới ngưỡng 0,5 với tên ngắn → vùng đúng **không vào nổi danh sách ứng viên**. Sửa ở `bde2d35`: nhánh current dùng tên đơn vị `parseAddress` đã tách (chỉ `ward`/`district`; `province` bị canonicalize nên không lấy). Đo lại production: `Phường Bàn Cờ` không-có-trong-10 → **0,801 hạng 1**, `Phường Diên Hồng` hạng 2 → **0,744 hạng 1**, có/không tiền tố cho cùng score; **hit@3 fuzzy 37/40 = baseline**, không hồi quy. Trong lúc chạy 9.4 cũng phát hiện và sửa **sự cố production 4 endpoint 503** (`malformed array literal`, `2728347`) — mục 0 hồ sơ.
- [x] **9.5** XONG trên staging dựng từ production (khớp từng bảng) — [hồ sơ](../../evidence/admin-alias/9-4-9-5-nghiem-thu-production.md). Checksum mapping theo **(relation ID vùng cũ → tên đích + tên tỉnh của đích, share, source)**, sắp trong JS, **không** dùng `admin_area.id` vì `admin.mjs` đánh lại ID bằng `row_number()`. Ba trạng thái cho **cùng một checksum `84f4cedbe29495c9`** và cùng mọi số đếm (alias 37.251, old 4.972, area 3.353, orphan 15, distinct_alias 19.254, mapped 37.236): nền → sau `admin-old.mjs` → sau `admin.mjs`. Kết luận: **`admin-old.mjs` chạy lẻ idempotent**, và **cập nhật dữ liệu thường bằng `admin.mjs` — đúng đường cron `data:update` — KHÔNG làm mất alias**, kể cả khi nó đánh lại toàn bộ ID `admin_area`. Đây là phép thử trực tiếp cho rủi ro cron thứ Hai 14/09 ghi trong DEVLOG: **rủi ro đó không còn**. Chốt an toàn của quyết định A cũng được chứng minh trong lần chạy thật: staging không có `poi` nên overlay bỏ phép đo mật độ và **không** gắn số đo nào, tức không tự chấp nhận gap khi thiếu bằng chứng. Ghi lại một lỗi trong phép đo của tôi: checksum đầu đổi giữa hai lần chạy chỉ vì `ORDER BY` theo tên không phải thứ tự toàn phần (**hai vùng hiện hành cùng tên "Xã Đại Đồng"**); `diff` cho thấy đúng 2 dòng cùng nội dung khác vị trí. Tin checksum mà không truy `diff` thì đã kết luận sai rằng pipeline không idempotent.
- [x] **9.6** Đường rollback **đã sẵn và đã kiểm**, chưa phải dùng: backup R2 `mapslibvn-20260907-0518.dump.zst` cộng backup trước publish `mapslibvn-20260907-1020.dump.zst`; restore + quyền `api` với bảng old đã thử trên DB cô lập `mapslibvn_restore_test` dựng từ schema/dữ liệu production thật (9.2); Worker bản trước vẫn còn để trả API/SDK/docs về. Luật giữ nguyên: khôi phục **cùng một generation** current+old+alias bằng workflow restore của repo, **không** chạy riêng `admin.mjs` cũ để xoá alias hay trộn ID, và **không** hạ schema 0008 khi còn quan hệ 1–n. Ghi nhận thêm từ 9.5: `admin.mjs` chạy lại cho **cùng checksum mapping**, nên một lần chạy lại pipeline không phải là đường rollback dữ liệu — nó không đổi gì.
- [x] **9.7** ĐÓNG 07/09/2026. Toàn bộ cổng xanh và DEVLOG có đủ bằng chứng bắt buộc — bản nghiệm thu hoàn tất ghi ở mục 4 DEVLOG: counts thực, coverage exceptions đã khai báo từng ca, checksum nguồn, fixture pass, precision, p50/p95/p99, commit, workflow URL, artifact rollback. **Phạm vi đóng phải đọc kèm:** cổng độ phủ xanh **trong phạm vi snapshot Geofabrik 250101 đã chứng minh** (4.215 relation cấp xã cũ, không phải ~10.000 đơn vị pháp lý); 23 chỗ thiếu do nguồn được khai báo có bằng chứng từng ca chứ không bị bỏ lặng lẽ, và ca nào không khai báo vẫn chặn phát hành. Phát hành npm để sau theo quyết định PHONG. **Điểm tiếp theo: viết plan hạng mục 3** — không tự implement phonetic, `viKey`, tsvector, `matched_alt` hay migration 0009.
