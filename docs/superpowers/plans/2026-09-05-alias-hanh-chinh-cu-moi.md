...
**Trạng thái:** Task 0–7 hoàn tất ngày 06/09/2026, gồm cổng EXPLAIN 6.5 đã đo ở quy mô toàn quốc ([hồ sơ](../../evidence/admin-alias/6-5-explain-autocomplete-area.md)) và SDK/docs lên 0.2.0. Task 8 (bộ kiểm chứng toàn quốc + benchmark) là bước kế tiếp, nhưng **bị chặn bởi việc 3** dưới đây. Hạng mục 2 đã có plan riêng và đã đóng. Chưa tuyên bố độ phủ toàn quốc vì ledger nguồn còn khoảng thiếu Khánh Hòa/Jan→Jun.

**Bốn việc đã phát hiện — hai đã sửa, hai còn mở:**
...
4. **[CÒN MỞ — xếp hạng]** **Gợi ý `area` gần như không bao giờ hiện với `types` mặc định.** Đo trên API local + fixture Q1 (Task 7.6): `Quận 10`, `Quận 3`, `Quận 4`, `Quận 5`, `Quận Bình Thạnh`, `Quận Phú Nhuận`, `Thành phố Thủ Đức` — **cả bảy** đều không có item `area` nào trong 10 gợi ý; gọi riêng `types=area` thì trả đúng vùng kèm bbox. Nguyên nhân **không** phải `prior` (chỉ nặng 0,05 trong `COEFF`) mà vì `area-candidates.ts` trả `0 AS pop`, nên vùng mất trắng thành phần `0,15·pop`: area được 0,76 còn POI khớp token đạt 0,875. Hệ quả: tính năng Task 6 coi như vô hình ở đường mặc định. Cần PHONG quyết hướng (cho vùng một `pop` tổng hợp, dành sẵn một suất cho `area`, hay tăng `prior` khi truy vấn phân giải được thành tên hành chính) — đây là đổi xếp hạng nên không tự làm. Docs `tinh-nang.md` đã ghi giới hạn này.
