...
- Attribution: luôn thêm `AttributionControl` với **chuỗi đầy đủ** `attributionHtml()`, không có tuỳ chọn tắt (chỉ có `compact`). Hai source trong style (`openmaptiles`, `poi`) mang **đúng cùng chuỗi đó**, sinh lúc build từ `@mapslibvn/core` nên không thể lệch. MapLibre gộp các chuỗi ghi nguồn trùng khít nhau, vì vậy người dùng chỉ thấy một lần. Thiết kế này để mỗi phía tự đủ: nạp style thẳng vào `maplibre-gl` thuần vẫn có ghi nguồn, còn phía SDK thì ẩn lớp POI (`poiLayer: false`), thiếu bản POI trong manifest, hay dùng style URL lạ đều không làm mất bên nào. **Không được** để chuỗi ở hai phía lệch nhau, dù chỉ một ký tự, vì khi đó ghi nguồn sẽ hiện hai lần. Sửa 04/09/2026.
