...
- Attribution: luôn thêm `AttributionControl`, không có tuỳ chọn tắt (chỉ có `compact`). Chuỗi thêm vào phụ thuộc style: với theme `light`/`dark` của MapsLibVN chỉ thêm `mapsLibVNAttributionHtml()` vì mỗi source trong style đã tự khai `attribution` của nó (thêm chuỗi đầy đủ sẽ hiển thị nguồn hai lần); với style URL tuỳ biến thì thêm `attributionHtml()` đầy đủ. Ghi nguồn trong style là bắt buộc và **không được gỡ**: style là endpoint công khai, ai nạp thẳng vào maplibre-gl thuần vẫn phải thấy nguồn. Sửa 04/09/2026. Hệ quả có chủ đích: khi manifest chưa có bản POI, `renderStyle` bỏ source `poi` nên bản đồ không hiện ghi nguồn Overture/Foursquare — đúng, vì lúc đó bản đồ không vẽ dữ liệu của hai nguồn này; nghĩa vụ ghi nguồn cho **kết quả API** vẫn thuộc ứng dụng nhúng qua `/v1/attribution`.
