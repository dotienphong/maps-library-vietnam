...
- **08/09/2026 — Đã duyệt spec và viết plan mở rộng ba profile POI, chờ chọn cách thực thi.**
  ...
  **Task 2 ĐÓNG:** release set dùng chung build ID; manifest nhận `--poi-profile profile=release`
  ...
  14/14. **Task 5 ĐÓNG:** style API chọn đúng archive/header cho `overture-fsq`, `overture`,
  `fsq`, và fallback về `all;fallback` nếu profile hợp lệ chưa có release. PostgreSQL API matrix
