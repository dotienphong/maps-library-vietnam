  ...
  return {
    ...
    Audio: {
      ...
      createAudioPlayer: vi.fn((_source?: unknown, _options?: unknown) => {
        const p = { loop: false, volume: 1, play: vi.fn(), pause: vi.fn(), remove: vi.fn() };
        players.push(p);
        return p;
      }),
    },
...
describe('expoAudioSession', () => {
  it('activate: đặt mode → kích hoạt session → phát vòng lặp im lặng (thứ tự bắt buộc để không câm khi nền)', async () => {
  ...
  it('activate hai lần liên tiếp không tạo player thứ hai (dùng lại, chỉ play() lại)', async () => {
    const a = expoAudioSession();
    await a.activate();
    await a.activate();
  ...
  it('deactivate: dừng + gỡ player, ngừng session, trả mode về mixWithOthers; lỗi ở bất kỳ bước nào đều nuốt', async () => {
    const a = expoAudioSession();
    await a.activate();
    const player = mocks.players[0];
    await a.deactivate();
