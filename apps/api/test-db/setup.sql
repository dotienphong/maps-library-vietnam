...
-- R5: fixture nguồn biệt lập (osm, fsq, user). Hai cụm tên dùng để kiểm cache theo cả hai thứ tự;
-- hàng "Foursquare Near" (fsq) đặt gần điểm hỏi reverse nhất để bắt lỗi chỉ lọc sau LIMIT 1
-- khi `sources=osm` tắt nguồn đó.
...
VALUES
  ...
  ('R5SOURCEFSQNEAR00000000001', 'R5 Profile Alpha Foursquare Near', 'r5 profile alpha foursquare near', 'cafe',
   ST_SetSRID(ST_MakePoint(107.00005, 11.00000), 4326), 'R5 Ward', 'R5 Province', 'R5 FSQ Near',
   90, 0.8, 'active', 'fsq', 'r5-fsq-near', 'pipeline'),
  ...
  ('R5SOURCEFSQNEAR00000000002', 'R5 Profile Beta Foursquare Near', 'r5 profile beta foursquare near', 'cafe',
   ST_SetSRID(ST_MakePoint(107.00005, 11.00010), 4326), 'R5 Ward', 'R5 Province', 'R5 FSQ Near',
   90, 0.8, 'active', 'fsq', 'r5-fsq-near-beta', 'pipeline'),
...
VALUES
  ('R5SOURCEFSQALIAS0000000001', 'R5 Alias Quy Nhơn Commercial', 'r5 alias quy nhon commercial',
   'cafe', ST_SetSRID(ST_MakePoint(107.001, 11.001), 4326), 'R5 Ward', 'R5 Province', 90, 0.8,
   'active', 'fsq', 'r5-alias-commercial', 'pipeline', NULL, NULL),
  ...
  ('R5SOURCEFSQTOKEN0000000001', 'R5 Token Commercial', 'zzqq vvxx commercial',
   'cafe', ST_SetSRID(ST_MakePoint(107.001, 11.002), 4326), 'R5 Ward', 'R5 Province', 90, 0.8,
   'active', 'fsq', 'r5-token-commercial', 'pipeline', to_tsvector('simple', 'r5 token branch'), NULL),
  ...
  ('R5SOURCEFSQKEY000000000001', 'R5 Key Commercial', 'wwqq zzxx commercial',
   'cafe', ST_SetSRID(ST_MakePoint(107.001, 11.003), 4326), 'R5 Ward', 'R5 Province', 90, 0.8,
   'active', 'fsq', 'r5-key-commercial', 'pipeline', NULL, 'r5canbien'),
  ...
  ('R5SOURCEFSQTELEX0000000001', 'Thư Viện Quốc Gia Commercial', 'thu vien quoc gia commercial',
   'cafe', ST_SetSRID(ST_MakePoint(107.001, 11.004), 4326), 'R5 Ward', 'R5 Province', 90, 0.8,
   'active', 'fsq', 'r5-telex-commercial', 'pipeline', NULL, NULL),
