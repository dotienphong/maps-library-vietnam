...
Scope `@mapslibvn` đã thuộc tài khoản dự án và bốn tarball `0.4.0` đã qua pack, quét secret, audit
và cài thử độc lập. Chưa chạy lệnh publish, nên hiện có **hai cách dùng**:
...
<Aside type="caution">
`@mapslibvn/web` **chưa có trên npm**. Lệnh dưới đây chỉ chạy được sau khi publish.
...
<Aside type="caution">
`@mapslibvn/react` cũng **chưa có trên npm**. Xem mục 3 để dựng tarball từ repo.
</Aside>
